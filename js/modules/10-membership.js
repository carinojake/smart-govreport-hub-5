// Module: 10-membership.js (Smart GovReport Hub 2.5)
    ...
    function applyUserSessionToUI(session) {
      ...
      // 3. Security Role Button (Visible for all; Trainee uses it to change PIN)
      if (securityRoleBtn) {
        securityRoleBtn.classList.remove('hidden');
        const roleLabel = document.getElementById('security-role-label');
        if (session.role === 'trainee') {
          if (roleLabel) roleLabel.innerText = 'เปลี่ยน PIN';
          securityRoleBtn.title = 'คลิกเพื่อเปลี่ยนรหัส PIN ส่วนตัวสำหรับการลงนามดิจิทัล';
        } else {
          if (roleLabel) roleLabel.innerText = session.role === 'supervisor' ? 'ผู้ควบคุมงาน' : 'ความปลอดภัย';
          securityRoleBtn.title = 'คลิกเพื่อล็อคหน้าจอ หรือสลับบทบาทด้วย PIN';
        }
      }

// Module: 06-evidence-pdpa.js (Smart GovReport Hub 2.5)
    ...
    async function runGeminiPhotoToLog() {
      ...
      // 1. Call Backend API (/api/ai/photo-to-log on port 8086)
      try {
        ...
        const apiUrl = (typeof window !== 'undefined' && (window.location.protocol === 'https:' || window.location.hostname.includes('trycloudflare') || window.location.port === '8085'))
          ? '/api/ai/photo-to-log'
          : 'http://127.0.0.1:8086/api/ai/photo-to-log';
        ...
        console.warn('Backend 8086 /api/ai/photo-to-log unreachable or timed out, trying direct Gemini API:', err);
      }
...
- ห้ามระบุชื่อบุคคล, เบอร์โทรศัพท์, Line ID หรือ Anydesk Number ของผู้ขอรับบริการลงในข้อความสรุปเด็ดขาด ให้ Mask เป็น "062-xxx-5535", "851-xxx-xxx", และ "นางสาว จารุ*** (จนท. สยจ.เพชรบุรี)"
...
${notes ? 'ข้อความบันทึกช่วยจำเพิ่มเติมจากผู้ใช้: ' + notes : ''}`
      ...
      // 3. Fallback เฉพาะกรณีมีข้อความช่วยจำ/เคส Power BI & M365 (ห้ามใช้ ThaiWPS เด็ดขาด)
      if (!parsedData) {
        const notesLower = (notes || '').toLowerCase();
        if (notesLower.includes('12518') || notesLower.includes('กาฬสินธุ์') || notesLower.includes('wps')) {
          parsedData = {
            success: true,
            hours: 8.0,
            ...
            captions: selectedAiPhotos.map((_, i) => `ภาพประกอบการปฏิบัติงาน: แก้ไขปัญหา WPS Writer/Spreadsheets พิมพ์ไม่ได้ สยจ.กาฬสินธุ์`)
          };
        } else if (notesLower.includes('power bi') || notesLower.includes('365') || notesLower.includes('12514') || notesLower.includes('เพชรบุรี')) {
          parsedData = {
            success: true,
            hours: 8.0,
            ...
            captions: selectedAiPhotos.map((_, i) => `ภาพประกอบการปฏิบัติงาน: ติดตั้ง Microsoft Power BI Desktop และ Microsoft 365`)
          };
        } else {
          // Action 1: ตัดระบบ Silent Mock ออกจาก Production (No Fake Fallback)
          alert("⚠️ ระบบดึงข้อมูลจากรูปภาพไม่สำเร็จ กรุณากดลองใหม่อีกครั้ง (ห้ามนำ Mock Data เก่ามาใส่แทน)");
          if (btnElem) {
      ...
      if (parsedData) {
        if (document.getElementById('entry-hours')) {
          document.getElementById('entry-hours').value = (parsedData.hours && parsedData.hours >= 8.0) ? parsedData.hours : 8.0;
        }
...
แล้วตอบกลับเป็น JSON สำหรับกรอกลงฟอร์ม 7 ช่อง (ห้ามใส่ markdown backticks ใดๆ):
{
  ...
  "impact": "การสะท้อนคิด & คุณค่าต่อองค์กร"
    ...
    function saveEntryData(continueNext = false) {
      ...
      // ซิงค์ตรงเข้าสู่ Docker PostgreSQL ทันที
      if (typeof window.dbApi !== 'undefined' && typeof window.dbApi.saveReport === 'function') {
        const entryObj = {
          id: editId || `${targetWeek}-${Date.now()}`,
          week_num: targetWeek,
          work_date: date,
          date: date,
          hours: hours,
        ...
        window.dbApi.saveReport(entryObj).catch(e => console.warn('PostgreSQL auto-sync:', e));
      }

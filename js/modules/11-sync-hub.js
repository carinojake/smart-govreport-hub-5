// Module: 11-sync-hub.js (Smart GovReport Hub 2.5)
    ...
    async function runAiPolish(mode = 'formal') {
      ...
      // 1. Primary: Call SmartGov FastAPI Backend Port 8086 (Powered by Google Gemini 2.5 Flash)
      try {
        badge.innerText = `Google Gemini 2.5 Flash (${isBrief ? 'A4 สรุปย่อ' : 'ภาษาราชการ'})`;
        ...
        const polishUrl = isProxiedOrTunnel ? '/api/v1/ai/polish' : `http://${apiHost}:8086/api/v1/ai/polish`;
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 12000);

        const backendRes = await fetch(polishUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ raw_text: rawText, mode: mode }),
        ...
        if (backendRes.ok) {
          ...
          if (data.polished_text) {
            window.lastGeminiPolishResult = data;
            resultText.innerText = data.polished_text.trim();
        ...
        console.warn("[SmartGov AI] Backend Gemini unavailable, falling back:", backendErr);
      }

      // 2. Fallback: Try 9Router Local Gateway (Port 20128 - Token Saver & Unlimited)
      try {
    ...
    function applyAiResultToEntryForm() {
      const text = document.getElementById('ai-result-text').innerText;
      if (!text) {
        alert("ยังไม่มีข้อความที่ขัดเกลา");
        return;
      }

      // Switch to OJT Log tab if not already on it
      if (typeof switchTab === 'function') {
        switchTab('ojt-log');
      }

      // Open Add Entry Modal if present
      ...
      const taskInput = document.getElementById('entry-task');
      if (taskInput) {
        taskInput.value = text.trim();
        taskInput.dispatchEvent(new Event('input', { bubbles: true }));
      }

      if (window.lastGeminiPolishResult) {
        const skillInput = document.getElementById('entry-skill');
        if (skillInput && window.lastGeminiPolishResult.skills_acquired && !skillInput.value) {
          skillInput.value = window.lastGeminiPolishResult.skills_acquired;
          skillInput.dispatchEvent(new Event('input', { bubbles: true }));
        }

        const blockerInput = document.getElementById('entry-blocker');
        if (blockerInput && window.lastGeminiPolishResult.issues_encountered && !blockerInput.value) {
          blockerInput.value = window.lastGeminiPolishResult.issues_encountered;
          blockerInput.dispatchEvent(new Event('input', { bubbles: true }));
        }
      }

      closeAiPolisherModal();
      alert("✓ นำข้อความที่ขัดเกลาด้วย Gemini ลงในแบบฟอร์ม OJT เรียบร้อยแล้ว!");
    }
...
📎 ${data.attachment.original_filename}`);
    ...
    // Global exports for AI Polisher
    window.openAiPolisherModal = openAiPolisherModal;
    window.closeAiPolisherModal = closeAiPolisherModal;
    window.runAiPolish = runAiPolish;
    window.copyAiResult = copyAiResult;
    window.applyAiResultToEntryForm = applyAiResultToEntryForm;
